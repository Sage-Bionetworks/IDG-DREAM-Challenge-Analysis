...
Let's rank the submissions, taking the best submission from R2 for each team (54 submissions total, then). Also, filter out teams that were disqualified. 
...
rank_preds_sp <- lapply(unique(preds$rank), function(i){
   ...
   foo <- preds %>%
     filter(rank <= i) %>% 
     mutate(predrankweight = (pred*(54-rank))/sum((54-i):53)) %>% 
     group_by(Compound_SMILES, Compound_InchiKeys, Compound_Name, UniProt_Id, Entrez_Gene_Symbol,
              DiscoveRx_Gene_Symbol) %>% 
...
ensemble_scores <- bind_rows(median_results_spearman, mean_results_spearman, rank_results_spearman)  %>%  
  set_names(c("ensemble_model", "score", "iteration")) %>% 
  mutate(iteration = as.numeric(iteration)) %>% 
  mutate(label = case_when(iteration == 1 & ensemble_model == "spearman_rank_ensemble"  ~ "Q.E.D.",
                           iteration == 2 & ensemble_model == "spearman_rank_ensemble"  ~ "1 + AI Winter is Coming",
                           iteration == 3 & ensemble_model == "spearman_rank_ensemble"  ~ "2 + DMIS_DK",
                           iteration == 4 & ensemble_model == "spearman_rank_ensemble"  ~ "3 + hulab",
                           iteration == 5 & ensemble_model == "spearman_rank_ensemble"  ~ "4 + METU_EMBL-EBI_CROssBAR",
                           iteration == 6 & ensemble_model == "spearman_rank_ensemble"  ~ "5 + PlosCB baseline",
                           iteration == 7 & ensemble_model == "spearman_rank_ensemble"  ~ "6 + Thin Nguyen",
                           iteration == 8 & ensemble_model == "spearman_rank_ensemble"  ~ "7 + KKT",
                           iteration > 8 | ensemble_model != "spearman_rank_ensemble" ~ ""))
...
plot the best vs gold standard where the 8th ensemble is the best: 

```{r echo=TRUE, message=FALSE, warning=FALSE}

best_ens_vs_gold <- mean_preds_sp %>% 
  left_join(gold) %>% 
  select(1:6, spearman_mean_ensemble_8, `pKd_[M]`) %>% 
  mutate(gold = `pKd_[M]`)

ggplot(data = best_ens_vs_gold) +
  geom_point(aes(x= gold, y= spearman_mean_ensemble_8)) +
  theme_bw() +
...
rank_preds_rm <- lapply(unique(preds$rank), function(i){
   ...
   foo <- preds %>%
     filter(rank <= i) %>% 
     mutate(predrankweight = (pred*(21-rank))/sum((21-i):20)) %>% 
     group_by(Compound_SMILES, Compound_InchiKeys, Compound_Name, UniProt_Id, Entrez_Gene_Symbol,
              DiscoveRx_Gene_Symbol) %>% 
     summarize(!!colname := sum(predrankweight)) %>% 
     ungroup()
...
ensemble_scores <- bind_rows(median_results_rmse, mean_results_rmse, rank_results_rmse)  %>%  
  ...
  mutate(label = case_when(iteration == 1 & ensemble_model == "rmse_rank_ensemble"  ~ "Gregory Koytiger",
                           ...
                           iteration > 4 | ensemble_model != "rmse_rank_ensemble" ~ ""))
...
```{r}
write_csv(mean_preds_sp,"spearman_mean_ensemble_models_qualified_subs.csv")
write_csv(mean_preds_rm,"rmse_mean_ensemble_models_qualified_subs.csv")
write_csv(median_preds_sp,"spearman_median_ensemble_models_qualified_subs.csv")
write_csv(median_preds_rm,"rmse_median_ensemble_models_qualified_subs.csv")
write_csv(rank_preds_sp,"spearman_rank_ensemble_models_qualified_subs.csv")
write_csv(rank_preds_rm,"rmse_rank_ensemble_models_qualified_subs.csv")

syn$store(synapse$File("spearman_mean_ensemble_models_qualified_subs.csv", parentId = "syn18700959"))
syn$store(synapse$File("rmse_mean_ensemble_models_qualified_subs.csv", parentId = "syn18700959"))
syn$store(synapse$File("spearman_median_ensemble_models_qualified_subs.csv", parentId = "syn18700959"))
syn$store(synapse$File("rmse_median_ensemble_models_qualified_subs.csv", parentId = "syn18700959"))
syn$store(synapse$File("spearman_rank_ensemble_models_qualified_subs.csv", parentId = "syn18700959"))
syn$store(synapse$File("rmse_rank_ensemble_models_qualified_subs.csv", parentId = "syn18700959"))
