...
p_1 <- ggplot(data_df_summary %>% group_by(exp_label, is_random_label, no_of_removed) %>% summarize(mean_rmse = mean(rmse), sd_rmse = sd(rmse)))+
  geom_col(aes(x = reorder(exp_label, no_of_removed), y = mean_rmse, fill = exp_label, alpha = is_random_label), stat = 'identity', position = position_dodge(preserve = "single")) +
  theme_bw() +
  labs(y= "RMSE", x = "Tanimoto Threshold") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  # facet_grid(cols = vars(is_random_label), space = "free", scales = "free") +
  geom_errorbar(aes(x = exp_label, y = mean_rmse, ymax = mean_rmse+sd_rmse, ymin = mean_rmse-sd_rmse, group = is_random_label, color = exp_label), position = "dodge") +
  ...
  scale_fill_manual(name = "pKd threshold",
                    values = c("no limit" = "#FDE725FF", 
                               "5 ≤ Kd ≤ no limit" = "#7AD151FF",
                               "5 ≤ Kd ≤ 9" = "#22A884FF", 
                               "5.5 ≤ Kd ≤ 8.5" = "#2A788EFF", 
                               "6 ≤ Kd ≤ 8" = "#414487FF",  
                               "no limit ≤ Kd ≤ 8" = "#440154FF")) +
    scale_color_manual(name = "pKd threshold",
                    values = c("no limit" = "#FDE725FF", 
                               ...
                               "6 ≤ Kd ≤ 8" = "#414487FF",  
                               "no limit ≤ Kd ≤ 8" = "#440154FF")) +
  scale_alpha_manual(name = "experiment condition", 
                     values = c('pKd dropout'=1,
                                'random control'=0.5)) +
...
p_2 <-ggplot(data_df_summary %>% group_by(exp_label, is_random_label, no_of_removed) %>% summarize(mean_spearman = mean(spearman), sd_spearman = sd(spearman)))+
  geom_col(aes(x = reorder(exp_label, no_of_removed), y = mean_spearman, fill = exp_label, alpha = is_random_label), stat = 'identity', position = position_dodge(preserve = "single")) +
  theme_bw() +
  labs(y= "Spearman correlation", x = "Tanimoto Threshold") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  # facet_grid(cols = vars(is_random_label), space = "free", scales = "free") +
  geom_errorbar(aes(x = exp_label, y = mean_spearman, ymax = mean_spearman+sd_spearman, ymin = mean_spearman-sd_spearman, group = is_random_label, color = exp_label), position = "dodge") +
...
p2 <- ggplot(qed_pkd_tidy) + 
  ...
  geom_segment(aes(x = 1, y = -750, xend = 14, yend = -750), 
               ...
               arrow = arrow(angle = 90, length = unit(0.1, "inches"),
                             ends = "both", type = "open"), color = "#FDE725FF", size = sz) +
  geom_segment(aes(x = 5, y = -4500, xend = 14, yend = -4500), 
               lineend = end, linejoin = join,
               arrow = arrow(angle = 90, length = unit(0.1, "inches"),
                             ends = "last", type = "open"), color = "#7AD151FF", size = sz) +
  geom_segment(aes(x = 5, y = -3750, xend = 9, yend = -3750), 
               lineend = end, linejoin = join, color = "#22A884FF", size = sz) +
  geom_segment(aes(x = 5.5, y = -3000, xend = 8.5, yend = -3000), 
               lineend = end, linejoin = join,  color = "#2A788EFF", size = sz) +
  geom_segment(aes(x = 6, y = -2250, xend = 8, yend = -2250), 
               lineend = end, linejoin = join, color = "#414487FF", size = sz) +
  geom_segment(aes(x = 1, y = -1500, xend = 8, yend = -1500), 
               lineend = end, linejoin = join,
               arrow = arrow(angle = 90, length = unit(0.1, "inches"),
                             ends = "first", type = "open"), color = "#440154FF", size = sz) +
